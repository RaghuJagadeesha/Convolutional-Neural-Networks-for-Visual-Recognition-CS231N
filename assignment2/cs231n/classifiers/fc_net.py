...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    hid1_out, hid1_cache = affine_relu_forward(X,W1,b1)
    scores, hid2_cache = affine_forward(hid1_out,W2,b2)
    ...
    dhid1_out, dW2, db2 = affine_backward(dscores, hid2_cache)
    dx, dW1, db1 = affine_relu_backward(dhid1_out, hid1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for layer in xrange(self.num_layers):
            self.params['W%d'%(layer+1)] = np.random.randn(dims[layer],dims[layer+1])*weight_scale
            self.params['b%d'%(layer+1)] = np.zeros(dims[layer+1])
    ...
    def loss(self, X, y=None):
        ...
        for i in xrange(1,self.num_layers):
            if self.use_batchnorm:
                ...
                layer[i],cache_layer[i] = affine_norm_relu_forward(layer[i-1], self.params['W%d'%i], \
                                                self.params['b%d'%i], gamma, beta, bn_params)            
            else:
        ...
        dx[self.num_layers], grads['W%d'%self.num_layers], grads['b%d'%self.num_layers] = affine_backward(dscores, cache_scores)
        ...
        for i in xrange(self.num_layers-1,0,-1):
            ...
            if self.use_batchnorm:
                dx[i],grads['W%d'%i],grads['b%d'%i], grads['gamma%d'%i], grads['beta%d'%i] = \
                                                         affine_norm_relu_backward(dx[i+1],cache_layer[i])
